            ...
            {events.map((ev) => (
              <Link key={ev.id} to={`/evento/${ev.id}`} className="block">
                <Card className="glass-card rounded-2xl border-border/60 hover:shadow-lg transition overflow-hidden">
                  {ev.image_url && (
                    <div
                      className="aspect-video bg-secondary/40"
                      style={{ backgroundImage: `url(${ev.image_url})`, backgroundSize: 'cover', backgroundPosition: 'center' }}
                    />
                  )}
                  <CardContent className="p-5 space-y-2">
                    <h3 className="font-display font-bold text-lg line-clamp-2">{ev.name}</h3>
                    {ev.event_date && <p className="text-sm text-muted-foreground flex items-center gap-1"><Calendar className="h-3.5 w-3.5" /> {formatEventDate(ev.event_date, ev.event_time)}</p>}
                    {ev.location && <p className="text-sm text-muted-foreground flex items-center gap-1"><MapPin className="h-3.5 w-3.5" /> {ev.location}</p>}
                  </CardContent>
                </Card>
              </Link>
            ))}
